...
108 in-match dice. An input-preserving validator requires a non-dash PA, copies
that state, selects the legal carrier activation and Pass declaration, requires
both transitions to consume zero dice, and requires at least one legal Pass
target occupied by a standing, tackle-zone-capable teammate without No Ball.
