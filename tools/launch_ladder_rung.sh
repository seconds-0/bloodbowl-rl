...
# graft only: admit zero-extended migration sidecars (validated by the screen).
...
if [ "$LADDER_PROFILE" != "graft" ] && {
  ...
  echo "GRAFT_FROM_SOURCE_SHA256/GRAFT_FROM_PATCH_BUNDLE_SHA256/GRAFT_REASON/GRAFT_ACCEPT_MIGRATED/GRAFT_MIGRATED_REASON require LADDER_PROFILE=graft" >&2
  exit 1
fi
