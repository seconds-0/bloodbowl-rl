...
TRAINER=$(pgrep -fc "puffer train" 2>/dev/null || echo 0)
echo "[$STAMP] tick: puffer_train_procs=$TRAINER" >>"$LOG"

PROMPT='You are the resident babysitter for the Blood Bowl RL rig (RTX 2070, ~190K SPS, a FREE experiment node). Read CLAUDE.md and the tail of DECISIONS.md for current program state and doctrine. CONTEXT: flagship training runs on remote vast boxes (NOT here); THIS box must stay 100% occupied with EXPERIMENTS (Alex directive) — a training arm should always be live. Before any puffer command: `source rig-env.sh` then activate vendor/PufferLib/.venv; this rig uses the --float build (precision_bytes=4, Turing sm_75) and the NATIVE backend (no --slowly, it is faster here). CHECK: is a `puffer train` process running (pgrep -fc "puffer train") AND is its newest /tmp/*.log advancing (mtime < 10 min)? If YES: log one status line (tag, steps, key metric) and exit. If NO / stale / crashed: inspect the newest /tmp/*.log, determine cap-vs-crash, and either relaunch the capped experiment advancing its ladder, or start the next queued experiment, per DECISIONS doctrine (pure-ladder curricula that GRADUATE to kickoff; reset-pct drills are RETIRED per D69). Append a 2-3 line summary of what you found and did to rig-babysitter.log. Be decisive and act now; do not ask questions.'
