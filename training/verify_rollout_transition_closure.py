...
def main() -> int:
    ...
    parser.add_argument(
        "--scope",
        choices=("native-cuda", "torch-cpu"),
        required=True,
        help="Execute exactly one backend contract; each requires its matching build.",
    )
    ...
    if args.scope == "native-cuda":
        if args.device != "cuda":
            raise SystemExit("native-cuda scope requires --device cuda")
        if not torch.cuda.is_available():
            raise SystemExit("CUDA verification requested but Torch CUDA is unavailable")
        verify_backend(_C, torch, "cuda")
        print(f"rollout transition closure: OK (native-cuda; {CONTRACT})")
    else:
        if args.device != "cpu":
            raise SystemExit("torch-cpu scope requires --device cpu")
        verify_torch_rollout_contract(torch)
        print(f"rollout transition closure: OK (torch-cpu; {CONTRACT})")
    return 0
