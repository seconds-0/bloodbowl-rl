...
- **Authored drill bank plan:** use
  ...
  proof privately verifies a non-dash-PA carrier's zero-die legal activation ->
  Pass declaration -> standing, tackle-zone-capable, non-No-Ball teammate
  target while serializing only the fresh-team-turn state. The raw match keeps
  its ordinary ball-carrier field, but no separate probe metadata, chosen
  action, target, or nested frame is serialized or labeled.
